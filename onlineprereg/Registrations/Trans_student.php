...
if (isset($_POST['register'])) {
        ...
        $folder1 = "trans-student-uploaded-files/".$abc;

        $agm = $_FILES["agm"]["name"];
        $tempname2 = $_FILES["agm"]["tmp_name"];    
        $folder2 = "trans-student-uploaded-files/".$agm;

        $af = $_FILES["af"]["name"];
        $tempname3 = $_FILES["af"]["tmp_name"];    
        $folder3 = "trans-student-uploaded-files/".$af;
        ...
        $check_lrn= mysqli_query($conn,"SELECT * FROM `trans_student` WHERE `lrn`='$lrn';");

        if (mysqli_num_rows($check_lrn)==0) {
            $sql = "INSERT INTO trans_student(lrn, name, age, gender, birthdate, address1, address2, height, weight, number, email, school, year, semester, grade, track, school_add, emergency_person, contact_num, guardians_name, g_address, g_cpnum, abc, agm, af, fname, occupation, cp, adde, ag, bd, mname, occu, cpn, addr, a, birth) VALUES 
            ('$lrn', '$name', '$age', '$gender', '$birthdate', '$address1', '$address2', '$height', '$weight' ,'$number', '$email',  '$school', '$year', '$semester', '$grade', '$track', '$school_add', '$emergency_person', '$contact_num', '$guardians_name', '$g_address', '$g_cpnum', '$abc', '$agm', '$af', '$fname', '$occupation', '$cp', '$adde', '$ag', '$bd', '$mname', '$occu', '$cpn', '$addr', '$a', '$birth')";
...
<html>
    ...
    <body>
            <div class="container">
                ...
                <div class="title">Transferee Student</div>
                <?php
                ...
                <form action="" class="action">
                    ...
                    <div class="student-details">
                        <div class="detail-box">
                            ...
                            <input type="text" maxlength="12" placeholder="Enter Learners Reference Number" required name="lrn">
                        </div>
                        <div class="detail-box">
                        ...
                        <div class="detail-box">
                            <span class="details">Grade Level</span><br>
                            <select name="grade">
                ...
                <div class="button">
                <button type="submit" name="register" class="btn btn-lg btn-primary" enabled>Register </button>
